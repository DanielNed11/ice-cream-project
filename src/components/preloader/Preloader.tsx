...
// two flavors prefetch quietly in the background after reveal. On small
// viewports (which render a static poster, not the scroll-scrub canvas --
// see SequenceCanvas) it only waits on that single poster image instead.
export function Preloader({ onDone }: PreloaderProps) {
  const defaultVariant = variants[0];
  const isDesktop = useIsDesktopViewport();
  const { loaded, total, done } = useSequencePreloader(defaultVariant, !isDesktop);
